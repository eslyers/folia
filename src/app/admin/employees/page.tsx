...
import { Plus, Users, Edit2, Trash2, Calendar, Save, X, Loader2 } from "lucide-react";
import { Header } from "@/components/Header";
import { Card, Button, Input } from "@/components/ui";
import { Modal } from "@/components/ui";
import { createClient } from "@/lib/supabase/client";
import type { Profile } from "@/lib/types";
import { format, differenceInYears, differenceInMonths, addYears, addMonths, startOfYear, endOfYear } from "date-fns";
...
export default function EmployeesPage() {
  ...
  const getVacationStats = (emp: any) => {
    ...
    // After 12 months, employee vests 30 days
    const hasVesting = totalMonths >= 12;
    
    // Needs attention if: has vesting but never took vacation OR balance is low
  ...
  const getVacationStatus = (emp: any) => {
    ...
    // Zero balance - needs vacation
    if (balance === 0) return { label: "Zerado", color: "text-red-600", variant: "error" as const };
    
    // Has vesting but high balance (never took vacation) - warning
